...
def get_ai_reasoning(company_name, price_change, start_date, end_date):
    ...
    data = {
        ...
        "messages": [
            ...
            {"role": "user",
             "content": f"Explain in 1-2 short sentences why {company_name} stock moved {price_change:.2f}% "
                        f"between {start_date} and {end_date}. Only mention specific news, events, or announcements "
                        f"that contributed to the rise or fall with full details. Do NOT speculate or say 'maybe'."}
        ],
        "temperature": 0.7
    }
    try:
