...
def get_ai_reasoning(company_name, price_change):
    ...
    data = {
        ...
        "messages": [
            ...
            {
                "role": "user",
                "content": f"Explain briefly why {company_name} stock moved {price_change:.2f}% between Aug 14 and Aug 29, 2025. Give specific info or news that lead to the price change."
            }
        ],
        "temperature": 0.7
    }
...
def display_top_movers_with_ai_only(performance, avg_volume, metadata, title, ascending=False):
    """
    Displays Top/Bottom 10 stocks with AI-generated reasoning only.
    """
    ...
    for i, row in df.iterrows():
        ...
        # Call AI to get reasoning
        reasoning = get_ai_reasoning(company, price_change)
...
with tab1:
    ...
    with col1:
        display_top_movers_with_ai_only(performance, avg_volume, metadata, "Top 10 Gainers", ascending=False)
    with col2:
        display_top_movers_with_ai_only(performance, avg_volume, metadata, "Top 10 Losers", ascending=True)

with tab2:
    # Optional: show sector/industry performance
