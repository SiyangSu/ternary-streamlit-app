...
def get_ai_reasoning(company_name, price_change, start_date, end_date):
    ...
    data = {
        ...
        "messages": [
            ...
            {"role": "user",
             "content": f"Explain briefly why {company_name} stock moved {price_change:.2f}% between {start_date} and {end_date}. Give specific info or news that led to the price change."}
        ],
        "temperature": 0.7
    }
    try:
...
def display_top_movers_with_ai(performance, avg_volume, metadata, title, start_date, end_date, ascending=False):
    ...
    for i, row in df.iterrows():
        ...
        st.markdown(f"**AI reasoning for {ticker} ({company}):** {reasoning}")
        st.markdown("---")
